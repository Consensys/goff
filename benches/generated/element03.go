...
// field modulus q =
//
// 3030655856750037494703694182318799474469281381387059293447
...
func (z *Element03) SetOne() *Element03 {
	z[0] = 14651112763505583602
	z[1] = 11972679449813671296
	z[2] = 634149879228807169
	return z
}
...
func (z *Element03) Neg(x *Element03) *Element03 {
	...
	var borrow uint64
	z[0], borrow = bits.Sub64(11121187691956759815, x[0], 0)
	z[1], borrow = bits.Sub64(3237032311947940159, x[1], borrow)
	z[2], _ = bits.Sub64(8906297097240372223, x[2], borrow)
	return z
}
...
func element03ModulusBigInt() *big.Int {
	onceelement03Modulus.Do(func() {
		_element03ModulusBigInt.SetString("3030655856750037494703694182318799474469281381387059293447", 10)
	})
	...
	for !uIsOne && !vIsOne {
		for v[0]&1 == 0 {
			...
			if s[0]&1 == 1 {
				...
				// s = s + q
				s[0], carry = bits.Add64(s[0], 11121187691956759815, 0)
				s[1], carry = bits.Add64(s[1], 3237032311947940159, carry)
				s[2], _ = bits.Add64(s[2], 8906297097240372223, carry)
		...
		for u[0]&1 == 0 {
			...
			if r[0]&1 == 1 {
				...
				// r = r + q
				r[0], carry = bits.Add64(r[0], 11121187691956759815, 0)
				r[1], carry = bits.Add64(r[1], 3237032311947940159, carry)
				r[2], _ = bits.Add64(r[2], 8906297097240372223, carry)
...
func (z *Element03) Sub(x, y *Element03) *Element03 {
	...
	if b != 0 {
		var c uint64
		z[0], c = bits.Add64(z[0], 11121187691956759815, 0)
		z[1], c = bits.Add64(z[1], 3237032311947940159, c)
		z[2], _ = bits.Add64(z[2], 8906297097240372223, c)
	}
	return z
}
...
func (z *Element03) FromMont() *Element03 {
	...
	{
		// m = z[0]n'[0] mod W
		m := z[0] * 5274690523073711945
		C := madd0(m, 11121187691956759815, z[0])
		C, z[0] = madd2(m, 3237032311947940159, z[1], C)
		C, z[1] = madd2(m, 8906297097240372223, z[2], C)
		z[2] = C
	}
	{
		// m = z[0]n'[0] mod W
...
func (z *Element03) Legendre() int {
	...
	// z^((p-1)/2)
	l.Exp(*z,
		14783965882833155715,
		10841888192828745887,
		4453148548620186111,
	)
	...
	// if l == 1
	if (l[2] == 634149879228807169) && (l[1] == 11972679449813671296) && (l[0] == 14651112763505583602) {
		return 1
	}
	return -1
}
...
func (z *Element03) Sqrt(x *Element03) *Element03 {
	...
	return z.Exp(*x,
		16615354978271353666,
		14644316133269148751,
		2226574274310093055,
	)
}

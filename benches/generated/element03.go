...
//
// 2593354290362757941043045148206313036395627720047652211673
//
...
func (z *Element03) SetOne() *Element03 {
	z[0] = 12945106739392473166
	z[1] = 16956608106207552303
	z[2] = 3204377483698730381
	return z
}
...
func (z *Element03) Neg(x *Element03) *Element03 {
	...
	var borrow uint64
	z[0], borrow = bits.Sub64(2750818667158539225, x[0], 0)
	z[1], borrow = bits.Sub64(745067983750999656, x[1], borrow)
	z[2], _ = bits.Sub64(7621183295005410617, x[2], borrow)
	return z
}
...
func element03ModulusBigInt() *big.Int {
	onceelement03Modulus.Do(func() {
		_element03ModulusBigInt.SetString("2593354290362757941043045148206313036395627720047652211673", 10)
	})
	...
	for !uIsOne && !vIsOne {
		for v[0]&1 == 0 {
			...
			if s[0]&1 == 1 {
				...
				// s = s + q
				s[0], carry = bits.Add64(s[0], 2750818667158539225, 0)
				s[1], carry = bits.Add64(s[1], 745067983750999656, carry)
				s[2], _ = bits.Add64(s[2], 7621183295005410617, carry)
		...
		for u[0]&1 == 0 {
			...
			if r[0]&1 == 1 {
				...
				// r = r + q
				r[0], carry = bits.Add64(r[0], 2750818667158539225, 0)
				r[1], carry = bits.Add64(r[1], 745067983750999656, carry)
				r[2], _ = bits.Add64(r[2], 7621183295005410617, carry)
...
func (z *Element03) Sub(x, y *Element03) *Element03 {
	...
	if b != 0 {
		var c uint64
		z[0], c = bits.Add64(z[0], 2750818667158539225, 0)
		z[1], c = bits.Add64(z[1], 745067983750999656, c)
		z[2], _ = bits.Add64(z[2], 7621183295005410617, c)
	}
	return z
}
...
// Exp z = x^e mod q
func (z *Element03) Exp(x Element03, e uint64) *Element03 {
	if e == 0 {
		return z.SetOne()
	}
...
func (z *Element03) FromMont() *Element03 {
	...
	{
		// m = z[0]n'[0] mod W
		m := z[0] * 9499640497684462487
		C := madd0(m, 2750818667158539225, z[0])
		C, z[0] = madd2(m, 745067983750999656, z[1], C)
		C, z[1] = madd2(m, 7621183295005410617, z[2], C)
		z[2] = C
	}
	{
		// m = z[0]n'[0] mod W

...
//
// 1051882114755502295300857442142668011025632611584520605510253318454932719511590793458820670682219
//
...
func (z *Element05) SetOne() *Element05 {
	z[0] = 3486539479758149418
	z[1] = 12552768824621053517
	z[2] = 6558290409039040942
	z[3] = 11574361540413253472
	z[4] = 278281587474121519
	return z
}
...
func (z *Element05) Neg(x *Element05) *Element05 {
	...
	var borrow uint64
	z[0], borrow = bits.Sub64(7480102296975701099, x[0], 0)
	z[1], borrow = bits.Sub64(12170359661399024857, x[1], borrow)
	z[2], borrow = bits.Sub64(15167598869190031144, x[2], borrow)
	z[3], borrow = bits.Sub64(3436191266648149071, x[3], borrow)
	z[4], _ = bits.Sub64(9084231243117715048, x[4], borrow)
	return z
}
...
func element05ModulusBigInt() *big.Int {
	onceelement05Modulus.Do(func() {
		_element05ModulusBigInt.SetString("1051882114755502295300857442142668011025632611584520605510253318454932719511590793458820670682219", 10)
	})
...
func (z *Element05) Inverse(x *Element05) *Element05 {
	...
	var u = Element05{
		7480102296975701099,
		12170359661399024857,
		15167598869190031144,
		3436191266648149071,
		9084231243117715048,
	}
	...
	for !uIsOne && !vIsOne {
		for v[0]&1 == 0 {
			...
			if s[0]&1 == 1 {
				...
				// s = s + q
				s[0], carry = bits.Add64(s[0], 7480102296975701099, 0)
				s[1], carry = bits.Add64(s[1], 12170359661399024857, carry)
				s[2], carry = bits.Add64(s[2], 15167598869190031144, carry)
				s[3], carry = bits.Add64(s[3], 3436191266648149071, carry)
				s[4], _ = bits.Add64(s[4], 9084231243117715048, carry)
		...
		for u[0]&1 == 0 {
			...
			if r[0]&1 == 1 {
				...
				// r = r + q
				r[0], carry = bits.Add64(r[0], 7480102296975701099, 0)
				r[1], carry = bits.Add64(r[1], 12170359661399024857, carry)
				r[2], carry = bits.Add64(r[2], 15167598869190031144, carry)
				r[3], carry = bits.Add64(r[3], 3436191266648149071, carry)
				r[4], _ = bits.Add64(r[4], 9084231243117715048, carry)
...
func (z *Element05) Sub(x, y *Element05) *Element05 {
	...
	if b != 0 {
		var c uint64
		z[0], c = bits.Add64(z[0], 7480102296975701099, 0)
		z[1], c = bits.Add64(z[1], 12170359661399024857, c)
		z[2], c = bits.Add64(z[2], 15167598869190031144, c)
		z[3], c = bits.Add64(z[3], 3436191266648149071, c)
		z[4], _ = bits.Add64(z[4], 9084231243117715048, c)
	}
	return z
}
...
// Exp z = x^e mod q
func (z *Element05) Exp(x Element05, e uint64) *Element05 {
	if e == 0 {
		return z.SetOne()
	}
...
func (z *Element05) FromMont() *Element05 {
	...
	{
		// m = z[0]n'[0] mod W
		m := z[0] * 3942641076184475581
		C := madd0(m, 7480102296975701099, z[0])
		C, z[0] = madd2(m, 12170359661399024857, z[1], C)
		C, z[1] = madd2(m, 15167598869190031144, z[2], C)
		C, z[2] = madd2(m, 3436191266648149071, z[3], C)
		C, z[3] = madd2(m, 9084231243117715048, z[4], C)
		z[4] = C
	}
	{
		// m = z[0]n'[0] mod W

...
// field modulus q =
//
// 56002438773231533402449956259495924226171041147936521373623944337188995348699
...
func (z *Element04) SetOne() *Element04 {
	z[0] = 13129780110385450570
	z[1] = 7702724659651757942
	z[2] = 6440463558940742071
	z[3] = 603337631044437670
	return z
}
...
func (z *Element04) Neg(x *Element04) *Element04 {
	...
	var borrow uint64
	z[0], borrow = bits.Sub64(11881854018516826331, x[0], 0)
	z[1], borrow = bits.Sub64(5372009707028896836, x[1], borrow)
	z[2], borrow = bits.Sub64(15226512294239180580, x[2], borrow)
	z[3], _ = bits.Sub64(8921703221332556972, x[3], borrow)
	return z
}
...
func element04ModulusBigInt() *big.Int {
	onceelement04Modulus.Do(func() {
		_element04ModulusBigInt.SetString("56002438773231533402449956259495924226171041147936521373623944337188995348699", 10)
	})
...
func (z *Element04) Inverse(x *Element04) *Element04 {
	...
	// initialize u = q
	var u = Element04{
		11881854018516826331,
		5372009707028896836,
		15226512294239180580,
		8921703221332556972,
	}
	...
	for !uIsOne && !vIsOne {
		for v[0]&1 == 0 {
			...
			if s[0]&1 == 1 {
				...
				// s = s + q
				s[0], carry = bits.Add64(s[0], 11881854018516826331, 0)
				s[1], carry = bits.Add64(s[1], 5372009707028896836, carry)
				s[2], carry = bits.Add64(s[2], 15226512294239180580, carry)
				s[3], _ = bits.Add64(s[3], 8921703221332556972, carry)
		...
		for u[0]&1 == 0 {
			...
			if r[0]&1 == 1 {
				...
				// r = r + q
				r[0], carry = bits.Add64(r[0], 11881854018516826331, 0)
				r[1], carry = bits.Add64(r[1], 5372009707028896836, carry)
				r[2], carry = bits.Add64(r[2], 15226512294239180580, carry)
				r[3], _ = bits.Add64(r[3], 8921703221332556972, carry)
...
func (z *Element04) Sub(x, y *Element04) *Element04 {
	...
	if b != 0 {
		var c uint64
		z[0], c = bits.Add64(z[0], 11881854018516826331, 0)
		z[1], c = bits.Add64(z[1], 5372009707028896836, c)
		z[2], c = bits.Add64(z[2], 15226512294239180580, c)
		z[3], _ = bits.Add64(z[3], 8921703221332556972, c)
	}
	return z
}
...
func (z *Element04) FromMont() *Element04 {
	...
	{
		// m = z[0]n'[0] mod W
		m := z[0] * 3961342386804948141
		C := madd0(m, 11881854018516826331, z[0])
		C, z[0] = madd2(m, 5372009707028896836, z[1], C)
		C, z[1] = madd2(m, 15226512294239180580, z[2], C)
		C, z[2] = madd2(m, 8921703221332556972, z[3], C)
		z[3] = C
	}
	{
		// m = z[0]n'[0] mod W
...
func (z *Element04) Legendre() int {
	...
	// if l == 1
	if (l[3] == 603337631044437670) && (l[2] == 6440463558940742071) && (l[1] == 7702724659651757942) && (l[0] == 13129780110385450570) {
		return 1
	}
	return -1
}

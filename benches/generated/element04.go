...
// field modulus q =
//
// 52504041189717816766988176258608108661331498956492267718959959362395383748163
...
func (z *Element04) SetOne() *Element04 {
	z[0] = 2370533237439681402
	z[1] = 15742175621540984444
	z[2] = 7875701457589474268
	z[3] = 1717991409488641601
	return z
}
...
func (z *Element04) Neg(x *Element04) *Element04 {
	...
	var borrow uint64
	z[0], borrow = bits.Sub64(17261477454989710915, x[0], 0)
	z[1], borrow = bits.Sub64(10575656262939059393, x[1], borrow)
	z[2], borrow = bits.Sub64(5285521308060038673, x[2], borrow)
	z[3], _ = bits.Sub64(8364376332110455007, x[3], borrow)
	return z
}
...
func element04ModulusBigInt() *big.Int {
	onceelement04Modulus.Do(func() {
		_element04ModulusBigInt.SetString("52504041189717816766988176258608108661331498956492267718959959362395383748163", 10)
	})
...
func (z *Element04) Inverse(x *Element04) *Element04 {
	...
	// initialize u = q
	var u = Element04{
		17261477454989710915,
		10575656262939059393,
		5285521308060038673,
		8364376332110455007,
	}
	...
	for !uIsOne && !vIsOne {
		for v[0]&1 == 0 {
			...
			if s[0]&1 == 1 {
				...
				// s = s + q
				s[0], carry = bits.Add64(s[0], 17261477454989710915, 0)
				s[1], carry = bits.Add64(s[1], 10575656262939059393, carry)
				s[2], carry = bits.Add64(s[2], 5285521308060038673, carry)
				s[3], _ = bits.Add64(s[3], 8364376332110455007, carry)
		...
		for u[0]&1 == 0 {
			...
			if r[0]&1 == 1 {
				...
				// r = r + q
				r[0], carry = bits.Add64(r[0], 17261477454989710915, 0)
				r[1], carry = bits.Add64(r[1], 10575656262939059393, carry)
				r[2], carry = bits.Add64(r[2], 5285521308060038673, carry)
				r[3], _ = bits.Add64(r[3], 8364376332110455007, carry)
...
func (z *Element04) Sub(x, y *Element04) *Element04 {
	...
	if b != 0 {
		var c uint64
		z[0], c = bits.Add64(z[0], 17261477454989710915, 0)
		z[1], c = bits.Add64(z[1], 10575656262939059393, c)
		z[2], c = bits.Add64(z[2], 5285521308060038673, c)
		z[3], _ = bits.Add64(z[3], 8364376332110455007, c)
	}
	return z
}
...
func (z *Element04) FromMont() *Element04 {
	...
	{
		// m = z[0]n'[0] mod W
		m := z[0] * 17546637072782121365
		C := madd0(m, 17261477454989710915, z[0])
		C, z[0] = madd2(m, 10575656262939059393, z[1], C)
		C, z[1] = madd2(m, 5285521308060038673, z[2], C)
		C, z[2] = madd2(m, 8364376332110455007, z[3], C)
		z[3] = C
	}
	{
		// m = z[0]n'[0] mod W
...
func (z *Element04) Legendre() int {
	...
	// z^((q-1)/2)
	l.Exp(*z,
		17854110764349631265,
		14511200168324305504,
		11866132690884795144,
		4182188166055227503,
	)
	...
	// if l == 1
	if (l[3] == 1717991409488641601) && (l[2] == 7875701457589474268) && (l[1] == 15742175621540984444) && (l[0] == 2370533237439681402) {
		return 1
	}
	return -1
}
...
func (z *Element04) Sqrt(x *Element04) *Element04 {
	...
	var y, square Element04
	y.Exp(*x,
		8927055382174815633,
		7255600084162152752,
		15156438382297173380,
		2091094083027613751,
	)
	square.Square(&y)
	if square.Equal(x) {

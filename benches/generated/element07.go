...
// field modulus q =
//
// 348349394757961897412802119268986676797171820892324363089895752170858524802526945970760265977759926123097254873018386703388494000185503
...
func (z *Element07) SetOne() *Element07 {
	z[0] = 12431521289225322178
	z[1] = 18091152780516051653
	z[2] = 3798694011303773980
	z[3] = 5219617364971277027
	z[4] = 6274212309851446572
	z[5] = 7681300952533310173
	z[6] = 764934014513227532
	return z
}
...
func (z *Element07) Neg(x *Element07) *Element07 {
	...
	var borrow uint64
	z[0], borrow = bits.Sub64(3007611392242114719, x[0], 0)
	z[1], borrow = bits.Sub64(9401167683451525789, x[1], borrow)
	z[2], borrow = bits.Sub64(7324025031202888817, x[2], borrow)
	z[3], borrow = bits.Sub64(15836935391223913102, x[3], borrow)
	z[4], borrow = bits.Sub64(6086265881929052521, x[4], borrow)
	z[5], borrow = bits.Sub64(14606093597442896529, x[5], borrow)
	z[6], _ = bits.Sub64(8840905029598162041, x[6], borrow)
	return z
}
...
func element07ModulusBigInt() *big.Int {
	onceelement07Modulus.Do(func() {
		_element07ModulusBigInt.SetString("348349394757961897412802119268986676797171820892324363089895752170858524802526945970760265977759926123097254873018386703388494000185503", 10)
	})
...
func (z *Element07) Inverse(x *Element07) *Element07 {
	...
	var u = Element07{
		...
		9401167683451525789,
		7324025031202888817,
		15836935391223913102,
		6086265881929052521,
		14606093597442896529,
		8840905029598162041,
	}
	...
	for !uIsOne && !vIsOne {
		for v[0]&1 == 0 {
			...
			if s[0]&1 == 1 {
				...
				// s = s + q
				s[0], carry = bits.Add64(s[0], 3007611392242114719, 0)
				s[1], carry = bits.Add64(s[1], 9401167683451525789, carry)
				s[2], carry = bits.Add64(s[2], 7324025031202888817, carry)
				s[3], carry = bits.Add64(s[3], 15836935391223913102, carry)
				s[4], carry = bits.Add64(s[4], 6086265881929052521, carry)
				s[5], carry = bits.Add64(s[5], 14606093597442896529, carry)
				s[6], _ = bits.Add64(s[6], 8840905029598162041, carry)
		...
		for u[0]&1 == 0 {
			...
			if r[0]&1 == 1 {
				...
				// r = r + q
				r[0], carry = bits.Add64(r[0], 3007611392242114719, 0)
				r[1], carry = bits.Add64(r[1], 9401167683451525789, carry)
				r[2], carry = bits.Add64(r[2], 7324025031202888817, carry)
				r[3], carry = bits.Add64(r[3], 15836935391223913102, carry)
				r[4], carry = bits.Add64(r[4], 6086265881929052521, carry)
				r[5], carry = bits.Add64(r[5], 14606093597442896529, carry)
				r[6], _ = bits.Add64(r[6], 8840905029598162041, carry)
...
func (z *Element07) Sub(x, y *Element07) *Element07 {
	...
	if b != 0 {
		var c uint64
		z[0], c = bits.Add64(z[0], 3007611392242114719, 0)
		z[1], c = bits.Add64(z[1], 9401167683451525789, c)
		z[2], c = bits.Add64(z[2], 7324025031202888817, c)
		z[3], c = bits.Add64(z[3], 15836935391223913102, c)
		z[4], c = bits.Add64(z[4], 6086265881929052521, c)
		z[5], c = bits.Add64(z[5], 14606093597442896529, c)
		z[6], _ = bits.Add64(z[6], 8840905029598162041, c)
	}
	return z
}
...
func (z *Element07) FromMont() *Element07 {
	...
	{
		// m = z[0]n'[0] mod W
		m := z[0] * 459208420298933409
		C := madd0(m, 3007611392242114719, z[0])
		C, z[0] = madd2(m, 9401167683451525789, z[1], C)
		C, z[1] = madd2(m, 7324025031202888817, z[2], C)
		C, z[2] = madd2(m, 15836935391223913102, z[3], C)
		C, z[3] = madd2(m, 6086265881929052521, z[4], C)
		C, z[4] = madd2(m, 14606093597442896529, z[5], C)
		C, z[5] = madd2(m, 8840905029598162041, z[6], C)
		z[6] = C
	}
	{
		// m = z[0]n'[0] mod W
...
func (z *Element07) Legendre() int {
	...
	// z^((p-1)/2)
	l.Exp(*z,
		10727177732975833167,
		13923955878580538702,
		3662012515601444408,
		17141839732466732359,
		12266504977819302068,
		16526418835576224072,
		4420452514799081020,
	)
	...
	// if l == 1
	if (l[6] == 764934014513227532) && (l[5] == 7681300952533310173) && (l[4] == 6274212309851446572) && (l[3] == 5219617364971277027) && (l[2] == 3798694011303773980) && (l[1] == 18091152780516051653) && (l[0] == 12431521289225322178) {
		return 1
	}
	return -1
}

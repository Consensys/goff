...
TEXT ·mulAssignElement(SB), NOSPLIT, $0-16
    ...
    // C,_ := t[0] + m*q[0]
    MULXQ ·modulusElement+0(SB), AX, R8
    ADCXQ CX, AX
    MOVQ R8, CX
    // for j=1 to N-1
    //     (C,t[j-1]) := t[j] + m*q[j] + C
    ...
    MOVQ BX, R8
    SBBQ ·modulusElement+8(SB), R8
    MOVQ BP, R10
    SBBQ ·modulusElement+16(SB), R10
    MOVQ SI, R9
    SBBQ ·modulusElement+24(SB), R9
    CMOVQCC R15, CX
    CMOVQCC R8, BX
    CMOVQCC R10, BP
    CMOVQCC R9, SI
...
no_adx:
    ...
    MOVQ R13, SI
    MOVQ CX, R8
    SUBQ ·modulusElement+0(SB), R8
    MOVQ BX, R10
    SBBQ ·modulusElement+8(SB), R10
    MOVQ BP, R9
    SBBQ ·modulusElement+16(SB), R9
    MOVQ SI, R11
...
TEXT ·fromMontElement(SB), NOSPLIT, $0-8
    ...
    ADOXQ AX, SI
    MOVQ CX, R9
    SUBQ ·modulusElement+0(SB), R9
    MOVQ BX, R10
    SBBQ ·modulusElement+8(SB), R10
    MOVQ BP, R11
    SBBQ ·modulusElement+16(SB), R11
    MOVQ SI, R12
    SBBQ ·modulusElement+24(SB), R12
    CMOVQCC R9, CX
    CMOVQCC R10, BX
    CMOVQCC R11, BP
    CMOVQCC R12, SI
...
TEXT ·squareElement(SB), NOSPLIT, $0-16
    ...
    MULXQ ·modulusElementInv0(SB), DX, AX                  // m := t[0]*q'[0] mod W
    XORQ AX, AX                                            // clear up flags
    MULXQ ·modulusElement+0(SB), AX, R12
    ADCXQ CX, AX
    MOVQ R12, CX
    ADCXQ BX, CX
    MULXQ ·modulusElement+8(SB), AX, BX
...
no_adx:
    ...
    MOVQ res+0(FP), R12
    MOVQ CX, R14
    SUBQ ·modulusElement+0(SB), R14
    MOVQ BX, R15
    SBBQ ·modulusElement+8(SB), R15
    MOVQ BP, R9
    SBBQ ·modulusElement+16(SB), R9
    MOVQ SI, R10
    SBBQ ·modulusElement+24(SB), R10
    CMOVQCC R14, CX
    CMOVQCC R15, BX
    CMOVQCC R9, BP
    CMOVQCC R10, SI
